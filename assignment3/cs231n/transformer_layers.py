...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        lens = torch.arange(0, max_len, dtype=torch.float)
        embedding = (-1 * torch.arange(0, embed_dim, 2) / embed_dim)
        pe_bef = torch.outer(lens, (10000 ** embedding))
        pe[0, :, 0::2] = torch.sin(pe_bef)
        pe[0, :, 1::2] = torch.cos(pe_bef)
