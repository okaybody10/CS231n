...
def rnn_step_backward(dnext_h, cache):
    ...
    dtmp = dnext_h * (1 - next_h) * (1 + next_h)
    dx = dtmp @ Wx.T
    dWx = x.T @ dtmp
    dprev_h = dtmp @ Wh.T
    dWh = prev_h.T @ dtmp
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_time = h0
    for i in range(T) :
        h_time, _ = rnn_step_forward(x[:, i, :], h_time, Wx, Wh, b)
        h[:, i, :] = h_time
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(1, T)) :
        dx_t, dprev_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_clone[:, i, :], (h[:, i, :], x[:, i, :], h[:, i-1, :], Wx, Wh, b))
        dx[:, i, :] = dx_t
        # if i < T-1 :
        #     dx[:, i, :] += dh[:, i+1] @ Wx.T
        dWx += dWx_t
        dWh += dWh_t
        db += db_t            
        dh_clone[:, i-1, :] += dprev_t
    dx_t, dh0, dWx_0, dWh_0, db_0 = rnn_step_backward(dh_clone[:, 0, :], (h[:, 0, :], x[:, 0, :], h0, Wx, Wh, b))
    dWx += dWx_0
    dWh += dWh_0
    db += db_0
    # dh0  = dh0 * dh[:, 0, :]
